...
install_nodejs() {
  ...
  # To do this, we override the typical version resolution method used by the built-pack to retrieve a a custom NodeJS build from Azure Blob.
  # The 'latest.txt' file has a link to the specific file on Azure Blob.
  if ! read number url < <(curl --silent --get --retry 5 --retry-max-time 15 --data-urlencode "range=$version" "https://rw20herokubuildpack.blob.core.windows.net/custom-node/latest.txt"); then
